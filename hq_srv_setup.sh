...
echo ""
echo "--- A-записи DNS (оставьте пустыми для использования стандартных) ---"
read -p "IP hq-srv.au-team.irpo [192.168.100.2]: " A_HQSRV
A_HQSRV=${A_HQSRV:-192.168.100.2}
read -p "IP hq-rtr.au-team.irpo (VLAN100) [192.168.100.1]: " A_HQRTR1
A_HQRTR1=${A_HQRTR1:-192.168.100.1}
read -p "IP hq-rtr.au-team.irpo (VLAN200) [192.168.200.1]: " A_HQRTR2
A_HQRTR2=${A_HQRTR2:-192.168.200.1}
read -p "IP hq-rtr.au-team.irpo (VLAN999) [192.168.99.1]: " A_HQRTR3
A_HQRTR3=${A_HQRTR3:-192.168.99.1}
...
timedatectl set-timezone "$TIMEZONE" 2>/dev/null || { apt-get install -y tzdata; timedatectl set-timezone "$TIMEZONE"; }
echo "  OK: $TIMEZONE"
...
# AllowUsers
sed -i "s/^#*AllowUsers .*/AllowUsers $SSH_USER/" $SSHD_CONF
grep -q "^AllowUsers " $SSHD_CONF || echo "AllowUsers $SSH_USER" >> $SSHD_CONF

# MaxAuthTries
sed -i "s/^#*MaxAuthTries .*/MaxAuthTries $MAX_AUTH/" $SSHD_CONF
grep -q "^MaxAuthTries " $SSHD_CONF || echo "MaxAuthTries $MAX_AUTH" >> $SSHD_CONF

# Banner
echo "$BANNER" > $BANNER_PATH
sed -i "s|^#*Banner .*|Banner $BANNER_PATH|" $SSHD_CONF
grep -q "^Banner " $SSHD_CONF || echo "Banner $BANNER_PATH" >> $SSHD_CONF
...
# Функция: IP -> PTR запись (разворачивает октеты)
# Пример: 192.168.100.1 -> 1.100.168.192.in-addr.arpa
ip_to_ptr() {
    local IP=$1
    local O1 O2 O3 O4
...
# Вычисляем PTR автоматически из введённых IP
PTR_HQRTR1=$(ip_to_ptr "$A_HQRTR1")
PTR_HQSRV=$(ip_to_ptr "$A_HQSRV")
PTR_HQRTR2=$(ip_to_ptr "$A_HQRTR2")
...
address=/hq-rtr.$DOMAIN/$A_HQRTR1
address=/hq-rtr.$DOMAIN/$A_HQRTR2
address=/hq-rtr.$DOMAIN/$A_HQRTR3
...
ptr-record=$PTR_HQRTR1,hq-rtr.$DOMAIN
ptr-record=$PTR_HQSRV,hq-srv.$DOMAIN
ptr-record=$PTR_HQRTR2,hq-rtr.$DOMAIN
