...
apt-get update -y && apt-get install -y iptables mc
echo "  OK"
...
# Указываем интерфейс для dhcpd
if [ -f /etc/sysconfig/dhcpd ]; then
    sed -i "s/^DHCPDARGS=.*/DHCPDARGS=\"$DHCP_IF\"/" /etc/sysconfig/dhcpd
    grep -q "^DHCPDARGS" /etc/sysconfig/dhcpd || echo "DHCPDARGS=\"$DHCP_IF\"" >> /etc/sysconfig/dhcpd
else
