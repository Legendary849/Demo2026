...
Проверка на клиенте:
- Убедитесь, что клиент получает IP-адрес из диапазона 192.168.200.10-192.168.200.20
...
При запросе ввода нажимайте Enter за исключением запроса пароля администратора.
Пароль администратора должен быть не менее 7 символов и содержать символы как минимум трёх групп из четырёх возможных.
...
Для того чтобы ввести HQ-CLI в домен - задаём статические параметры адресации, чтобы явно указать в качестве DNS-сервера IP-адрес BR-SRV.
...
После ввода в домен необходимо перезагрузить виртуальную машину HQ-CLI:
```bash
reboot
```
...
Просматриваем наши физические диски:
```bash
lsblk
```
...
Необходимо установить пакет ansible и sshpass:
```bash
apt-get update && apt-get install -y ansible sshpass
```

Приведём файл инвентаря ansible к следующему виду:
```bash
...
Теперь нужно подключить предварительно по ssh к каждому устройству:
```bash
...
- Импортируйте образы в docker, укажите в yaml файле параметры подключения к СУБД, имя БД - testdb, пользователь testc с паролем P@ssw0rd, порт приложения 8080
...
Выполнить монтирование Additional.iso:
```bash
mount /dev/sr0 /mnt/
```

Выполнить импорт образов:
```bash
...
Добавить записи в /etc/hosts (если DNS не настроен):
```bash
vim /etc/hosts
```
